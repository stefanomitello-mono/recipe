...
function render(name, image_url, publisher, id) {
    var item_template = document.getElementById("resultItem").content;

    var clone = document.importNode(item_template, true);
    clone.querySelector(".result__item--name ").textContent = truncate(name);
...
async function getSearchValue() {
    ...
    if ('content' in document.createElement('template')) {
        ...
        res.data.recipes.forEach(recipe => {
            results.appendChild(render(recipe.title, recipe.image_url, recipe.publisher, recipe.id));
        })
...
getRecipe('5ed6604591c37cdc054bc886')
    .then(data => {
