...
async function getSearchValue(event) {
    ...
    let s = document.getElementById('search-value').value
    console.info("search", s)

    let res = await searchRecipes(s);

    console.log("raw json: ", res)
    console.log("json paginated: ", paginator(res.data.recipes, 1))

    res = paginator(res.data.recipes, 1)
...
document.querySelector('.recipe__btn-next').addEventListener("click", getSearchValue);
